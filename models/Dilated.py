...
class Config(object):
    ...
    def __init__(self, dataset, embedding):
        ...
        self.hidden_structs = [128] * 5                                 # 扩张层结构
        self.num_layers = 2  # lstm层数
        self.dilations = [1, 2, 4, 8, 16]                                         # 扩张指数
        self.input_dim = 300
        self.cell_type = 'LSTM'
...
class Model(nn.Module):
    ...
    def __init__(self, config):
        ...
        #     [nn.Conv2d(1, config.num_filters, (k, config.hidden_size*2)) for k in config.filter_sizes])

        # 注意力ta用这个
        self.conv1 = nn.Conv1d(in_channels=config.hidden_size*2, out_channels=config.hidden_size, kernel_size=3)
        # 注意力H用这个
        ...
        #self.fc = nn.Linear(config.num_filters * len(config.filter_sizes), config.num_classes)0
    ...
    def forward(self, x):
        ...
        # ta = [batch_size,seq_len,hidden_size]
        ta = torch.cat(layer_outputs, dim=1).reshape(self.batch_size, self.n_steps, self.hidden_structs[0])
        M = self.tanh1(ta)  #     [batch_size, seq_len, hidden_size] =[16, 32, 128]
        # # M = torch.tanh(torch.matmul(H, self.u))
        alpha = F.softmax(torch.matmul(M, self.w), dim=1).unsqueeze(-1)  # [batch, seq_len, 1]
        out = ta * alpha  # [20, 32, 256] = [batch_size,seq_len,hidden]
        #out = out.unsqueeze(1)
        #out = torch.cat([self.conv_and_pool(out, conv) for conv in self.convs], 1)
        #out = torch.sum(out, 1)  # [20, 128]=[batch_size,hidden_size]
        # 注意力ta用这个
        cat_out = self.cnns_concat(out,self.hidden_structs[0],self.n_steps)
        # 注意力H用这个
        ...
        #activate_outputs = F.relu(pool1d_value)
